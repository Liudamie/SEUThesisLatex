...
新的测距表结构有效解决了由于时间戳缺失而导致的测距失败问题，其对应的权衡测距方法如算法\ref{alg:trade-off} 所示。
    ...
    \begin{algorithm}[H]
      \caption{Trade-off Ranging Algorithm}
      \label{alg:trade-off}
      \begin{algorithmic}[1]
      
      \While{true}  \Comment{parallel task: transmission}
          \State Delay for $P$ time
          \ForAll{neighbor $Y$}
...
\Statex
      \While{true} \Comment{parallel task: reception}
          \State Wait until a ranging message is received
          \State Let $Y$ be the message sender's ID
          \If{$R_f==\varnothing$ in Message}
              \State $T_r\gets T_{Y_{k-1}}, R_f\gets \varnothing, R_e\gets R_{Y}$ for table(AY)
              \State Compute Distance using extended timestamps
              \State rearrange Table: $R_r\gets R_{e}$ for table(AY)
          \Else
              \State $T_r\gets T_{Y_{k-1}}, R_f\gets R_{Y_{k}}, R_e\gets R_{Y}$ for table(AY)
              \State Compute Distance
              \State rearrange Table: $T_h\gets T_{Y_{k-1}}, R_h \gets R_{Y_{k-1}}, R_p\gets R_{A_{k}}, T_p\gets T_{A_{k}}, R_r\gets R_{e}$ for table(AY)
          \EndIf
      \EndWhile
...
Compute $tx, rx$ by Eq.(\ref{eq:txrxdiff}) and (\ref{eq:rxtxdiff}) \\
Init $recv\_packets = [\,]$ and $seq = 1$ \\
...
平均测距周期为60 ms，抖动设置为6。
飞行过程如图\ref{fig:3_20}所示，四张图分别展示了无人机的初始位置、跟随阶段以及编队变化阶段。
...
此外，在编队飞行过程中，记录了4架无人机相对于橙色圆圈（编号为2的无人机）的UWB测距数据及地面真实距离，如图\ref{fig:3_21}(a)所示。
图\ref{fig:3_21}显示了测距误差分布，两架无人机之间距离测量的平均绝对误差约为6.21 cm。
