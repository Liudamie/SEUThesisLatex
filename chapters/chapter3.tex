...
针对上述问题，本文首先提出了一种面向相对定位的鲁棒集群测距协议。
...
\begin{figure}[htbp]
  \centering
  \subfloat[带有时间戳的报文交换\label{fig:3_2a}]{\includegraphics[width=.45\textwidth]{figures/content/chapter3/mes_ex_A_Y.png}}
  \quad\quad
  \subfloat[计算距离的集群测距表\label{fig:3_2b}]{\includegraphics[width=.38\textwidth]{figures/content/chapter3/rangingTable.png}}
...
鲁棒集群测距协议需要在尽可能减少连续测距失败次数的同时，尽量降低权衡测距的发生频率。
因此，本文将抖动参数选择问题建模为多目标优化问题，并采用简单加权法来选择合适的抖动参数。
...
\begin{theorem}[最优抖动参数选择]
在该框架下，可得到抖动参数的最优选择规律如下。
在上述多目标优化模型下，当权重系数取 $\alpha=0.9$ 时，最优抖动参数 $W^*$ 满足一阶最优性条件
  \begin{equation}
    \begin{aligned}
    \frac{\partial F(W, P)}{\partial W}=0, F(W,P)=\alpha F_{G}(W)+\beta F_{H}(W) 
    \end{aligned}
    \end{equation} 
且其近似解为$W \approx P/10$。
\end{theorem}
\begin{proof}
在约束区间 $0 < W < \frac{2}{3}P$ 内，归一化函数 $F_G(W)$ 与 $F_H(W,P)$ 均连续可导，因此加权目标函数
  \begin{equation}
F(W,P)=\alpha F_{G}(W)+\beta F_{H}(W)
    \end{equation} 
在该区间内连续可导。

由于 $F_G(W)$ 与 $F_H(W,P)$ 分别刻画两个相互竞争的性能指标，且关于 $W$ 变化趋势相反，故 $F(W,P)$ 在区间内部存在极小值点。根据连续可导函数的极值必要条件，最优解 $W^*$ 满足
$\frac{\partial F(W, P)}{\partial W}=0$。

  为有效降低连续测距失败的平均次数，本文将权重系数 $\alpha$设定为 0.9。进一步结合数值近似分析,最终可求得最优抖动参数为$W \approx P/10$。
  \end{proof}
...
本节通过真实无人机集群实验，对所提出的鲁棒集群测距协议以及自适应相对定位算法的实际性能进行验证，并进一步展示在仅依赖相对定位信息条件下的大规模无人机集群自主飞行能力。

实验平台采用开源微型四旋翼无人机 Crazyflie 2.1 构建无人机集群系统。
...
\item 超宽带测距模块（DW3000 Deck）：基于 DWM3000 UWB芯片，用于无人机之间的距离测量，是实现集群相对定位的重要信息来源。
...
所有实验均在 6.9 m × 7.2 m 的室内实验场地中进行，如图\ref{fig:3_13}所示。
为了获得精确的参考数据，实验系统使用Vicon运动捕捉系统实时获取每架无人机的真实位置与偏航角。
...
通过大量真实飞行实验验证，所提出的鲁棒集群测距协议能够在动态密集的无人机网络环境中提供稳定、高效的距离更新，从而显著提升自适应相对定位算法在大规模无人机集群中的定位性能与鲁棒性。
...
\begin{figure}[h]
\centerline{\includegraphics[width=0.8\textwidth]{figures/content/chapter3/robotsSimulation.pdf}}
\caption{
不同规模无人机集群测距性能对比仿真}
\label{fig:3_14}
\end{figure}
...
\subsection{自适应相对定位算法性能}

为了评估所提出自适应相对定位算法的定位精度与稳定性，本文首先通过仿真实验对算法性能进行验证，并与传统扩展卡尔曼滤波方法进行对比分析。
...
对比两图的上部位置跟踪曲线可以观察到：传统 EKF在目标无人机执行 1 m × 1 m 正方形轨迹的转弯机动阶段（约 $t = 2.5$ s、$5.0$ s、$7.5$ s 等时刻），估计轨迹与真实轨迹之间存在肉眼可辨的滞后与超调现象。尤其在 Y 轴方向，当无人机由上升段转入平飞段时，EKF 估计值未能及时响应运动状态的变化，导致橙色曲线明显偏离蓝色真实值。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=.6\linewidth]{figures/content/chapter3/轨迹对比图.png}
  \caption{自适应EKF估计与真实轨迹对比图}
  \label{fig:3_22}
\end{figure}
图\ref{fig:3_22}展示了真实轨迹与AEKF估计轨迹在XY平面上的对比。
可以看出，AEKF估计轨迹与真实轨迹在整体趋势上高度吻合，尤其是在直线运动段，估计误差较小。在转弯或曲率变化较大的区域，估计轨迹仍能较好地跟随真实轨迹，未出现明显发散或漂移现象。
...
实验中基于上文的自适应相对定位算法实现无人机之间的相对定位。
实验共使用13架Crazyflie无人机，初始时各无人机间距为1米。
在飞行过程中，9架无人机在边长为2 m的内环区域飞行，其余4架无人机在边长为4 m的外环区域飞行。
平均测距周期为60 ms，抖动设置为6。
飞行过程如图\ref{fig:3_23}所示，四张图分别展示了无人机的初始位置、跟随阶段以及编队变化阶段。
...
此外，在编队飞行过程中，记录了4架无人机相对于橙色圆圈（编号为2的无人机）的UWB测距数据及地面真实距离，如图\ref{fig:3_24}(a)所示。
图\ref{fig:3_24}显示了测距误差分布，两架无人机之间距离测量的平均绝对误差约为6.21 cm。
...
实验结果表明，所提出的无人机集群自适应相对定位算法在动态且密集的大规模无人机群中，能够实现稳健且精确的距离测量。
因此，该算法可为大规模无人机群的高效定位和自主编队飞行任务提供可靠支持。

\section{本章小节}

本章针对无人机集群相对定位问题，提出了一种基于 UWB 的自适应相对定位方法。
